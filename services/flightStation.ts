...
export const getAirportInfo = async (): Promise<AirportInfo> => {
    ...
    // Si falla, y el usuario NO quiere datos de ejemplo:
    console.warn('No se pudieron obtener datos reales de AENA.');

    // Devolver estructura vacía pero marcada como "real" (o fallida) para no inventar datos
    ...
    return {
        ...
        llegadas: [],
        salidas: [],
        isRealData: true // Mantenemos true para indicar que NO es ejemplo, aunque esté vacío
    };
};

/**
 * Intenta obtener datos reales del aeropuerto
 */
const tryGetRealData = async (): Promise<AirportInfo | null> => {
    try {
        // Método 1: Usar un servicio proxy si está configurado
        const proxyUrl = import.meta.env.VITE_FLIGHT_PROXY_URL || 'http://localhost:3002';
        if (proxyUrl) {
            try {
                const response = await fetch(`${proxyUrl}/airport/${SEVILLA_AIRPORT_CODE}`, {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok) {
                    ...
                    const parsedData = parseAenaData(data);

                    // Si no hay vuelos, considerar que falló la obtención de datos reales
                    if (parsedData.llegadas.length === 0 && parsedData.salidas.length === 0) {
                        return null;
                    }

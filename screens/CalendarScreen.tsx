...
const CalendarScreen: React.FC<CalendarScreenProps> = ({ navigateTo }) => {
    ...
    return (
        <div className="bg-zinc-950 h-screen flex flex-col overflow-hidden fixed inset-0 w-full px-3 pt-3">
            <ScreenTopBar
                title="Calendario"
                ...
                {weekDays.map((day, index) => (
                    <div
                        ...
                        className={`text-center text-xs font-medium ${
                            index >= 5 ? 'text-orange-500' : 'text-cyan-400'
                        }`}
                    >
                        {day}
                    </div>
                            ...
                            {dayLetters[day] && (
                                <div className="absolute top-0.5 left-0.5">
                                    <span className={`text-xs font-bold px-1 py-0.5 rounded flex-shrink-0 ${
                                        (isSelected || isToday) ? 'bg-white text-zinc-900' : 'bg-blue-600 text-white'
                                    }`}>
                                        {dayLetters[day]}
                                    </span>
                                </div>
            ...
            {/* Footer con icono Home */}
            <div className="bg-blue-800 py-2 flex items-center justify-center flex-shrink-0">
                <button

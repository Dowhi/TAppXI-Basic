...
interface ReportFilter {
    ...
    gastosFiltro?: 'todos' | 'vehiculo' | 'iva' | 'conceptos' | 'proveedores';
    vehiculo?: string;
    concepto?: string;
    proveedor?: string;
}
...
const ReportsScreen: React.FC<ReportsScreenProps> = ({ navigateTo }) => {
    ...
    const generarInforme = async () => {
        ...
        try {
            ...
            if (filtros.tipo === 'gastos' || filtros.tipo === 'todos') {
                if (filtros.gastosFiltro === 'vehiculo' && filtros.vehiculo) {
                    gastosFinales = gastosFinales.filter(g => g.taller === filtros.vehiculo);
        ...
        if (filtros.tipo === 'gastos' || filtros.tipo === 'todos') {
            ...
            if (gastos.length > 0) {
                ...
                if (typeof autoTableModule.default === 'function') {
                    ...
                    autoTableModule.default(doc, {
                        startY: yPos,
                        head: [['Fecha', 'Concepto', 'Proveedor', 'Base Imponible', 'IVA %', 'IVA €', 'Total', 'Nº Factura', 'Forma Pago']],
                        body: gastosData,
                        theme: 'striped',
    ...
    const obtenerValoresUnicos = async () => {
        try {
            ...
            const vehiculos = [...new Set(gastos.map(g => g.taller).filter(Boolean))];
            ...
            return { vehiculos, conceptos, proveedores };
        } catch (error) {
    ...
    const [valoresUnicos, setValoresUnicos] = useState<{
        ...
        conceptos: string[];
        proveedores: string[];
    }>({ vehiculos: [], conceptos: [], proveedores: [] });
                    ...
                    {(filtros.tipo === 'gastos' || filtros.tipo === 'todos') && (
                        <>
                            ...
                            {filtros.gastosFiltro === 'vehiculo' && (
                                <div className="mb-4">
                                    <label className="block text-zinc-300 text-sm mb-2">Vehículo</label>
                                    <select
                                    ...
                                    >
                                        ...
                                        {valoresUnicos.vehiculos.map(veh => (
